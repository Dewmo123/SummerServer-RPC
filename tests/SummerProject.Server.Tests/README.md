...
| `Characters` | 캐릭터 지연 생성, 경험치 경계·오버플로·경쟁 테스트 |
| `Currencies` | 재화 지연 생성·정렬과 원자적 증감·경쟁 테스트 |
| `Stages` | 공개 조회, 입장 교체, 완료 선점·보상·롤백 테스트 |
| `Rooms` | 방 저장·조회, 함정 검증, 크기 경계와 동시 Upsert 테스트 |
| `Gameplay` | 캐릭터·재화 테스트의 공통 지원 코드와 보상 트랜잭션 조합 테스트 |
